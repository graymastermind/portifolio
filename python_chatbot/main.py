...
def reply_to_sms():
    """Respond to incoming SMS with the appropriate message."""
    ...
    if incoming_message.lower() == 'hi':
        ...
        if existing_user and not existing_user[1]:
            ...
            cursor.execute("UPDATE users SET username=?, user_id=? WHERE phone_number=?", (username, user_id, request.form['From']))
            get_db().commit()

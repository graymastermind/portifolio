...
const AppResize = (e) =>{
	...
	if(appObj.mainWidth<appObj.mainHeight){	
		...
		appMc.armour.x = 0;
		appMc.armour.y = -120+appObj.canvasHeight*0.5/appMc.mcUI.scale.y;
		appMc.armour.scale.set(0.5)

		appMc.attributes.scale.set(0.6)
		...
		TweenLite.killTweensOf(appMc.startBattle);
		appMc.startBattle.scale.set(0.3)
		gsap.from(appMc.startBattle.scale, 0.2,{delay:0.2, overwrite:"none", x:0, y:0, ease:Sine.easeOut});
		gsap.to(appMc.startBattle.scale, 0.5,{delay:0.4, yoyo:true, repeat:-1, overwrite:"none", x:"-=0.05", y:"-=0.05", ease:Sine.easeInOut});
		...
		appMc.characters2.y = -15
		appMc.characters2.x = 0

		appMc.armour.scale.set(0.4)

		appMc.attributes.scale.set(0.5)
		appMc.attributes.x = -280+appObj.canvasWidth*0.5/appMc.mcUI.scale.y;
		...
		appMc.startBattle.x = 0;
		appMc.startBattle.y = -183+appObj.canvasHeight*0.5/appMc.mcUI.scale.y;
		appMc.startBattle.scale.set(0.3)
